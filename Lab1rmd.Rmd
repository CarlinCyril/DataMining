...
```
\
The cloud of points $(X_{1,i}, x_{2,i})_i$ is very close to a straight line because $X_2$ is defined as a linear  function of $X_1$.
...
```
\
For both models, the two predictors can be assessed as non-zero at level 1% (thanks to their p-value). It is thus reasonable to state that, for each model, the two predictors are necessary to describe the model.
Furthermore, each model has a very low p-value (around $10^{-5}$) which supports the previous statements.
...
Also, their estimates were almost two times higher ($\hat\beta_1 = 4.0861$ and $\hat\beta_2 = 1.4239$) comparing to their estimates of this model where we try to explain $Y$ with $X_1$ and $X_2$ at the same time ($\hat\beta_1 = 2.0628$ and $\hat\beta_2 = 0.7336$).\
